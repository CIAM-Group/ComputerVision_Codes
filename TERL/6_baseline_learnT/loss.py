...
class KCL_ori(nn.Module):
    ...
    def forward(self, logits, im_labels, queue_label):
        ...
        mask_pos_view_class = torch.cat([torch.ones([mask_pos_view_class.shape[0], 1]).cuda(), mask_pos_view_class],
                                        dim=1)
...
class KCL(nn.Module):
    def __init__(self, args, K, k=6, temperature=0.07):
        super(KCL, self).__init__()
        self.args = args
        self.K = K
        self.k = k
    ...
    def forward(self, logits, labels, queue_label):
        # idx = torch.where(labels.sum(dim=-1) > 0)[0]
        # if len(idx)==0:
        #     return None
        # logits = logits[idx, :]
        # labels = labels[idx, :]
        ...
        # loss = 0
        # labels = im_labels.chunk(2)[0]
        # queue_label = queue_labels[0]

        mask = torch.eq(labels, queue_label)
        # print(str(mask.sum(dim=-1)))
        mask_pos_view = torch.zeros_like(mask)
        # positive logits from queue
        num_positive = self.k
        if num_positive > 0:
        ...
        mask_pos_view = torch.cat([torch.ones([mask_pos_view.shape[0], 1]).cuda(), mask_pos_view], dim=1)
        # print(mask_pos_view.sum(dim=-1))
        log_prob = F.normalize(logits.exp(), dim=1, p=1).log()
        loss_con = - torch.sum((mask_pos_view * log_prob).sum(1) / mask_pos_view.sum(1)) / \
                   mask_pos_view.shape[0]

        return loss_con


class KCLProto(nn.Module):
    def __init__(self, args, K, k=6, temperature=0.07):
        super(KCLProto, self).__init__()
        self.args = args
        self.K = K
        self.k = k
        self.temperature = temperature

    def forward(self, logits, queue_label):
        idx = torch.where(queue_label.sum(1) > 0)[0]
        if len(idx) == 0:
            loss_con = 0
        else:
            logits = logits[idx, :]
            queue_label = queue_label[idx, :]
            logits = logits / self.temperature
            # loss = 0
            # labels = im_labels.chunk(2)[0]
            # queue_label = queue_labels[0]
...
class SupConLoss_rank(nn.Module):
    ...
    def forward(self, logits, im_label, queue_label, im_cluster, queue_cluster):
        ...
        loss_cluster = - torch.sum((cluster_mask_com * log_cluster_prob).sum(1) / cluster_mask_com.sum(1)) / \
                       cluster_mask_com.shape[0]
