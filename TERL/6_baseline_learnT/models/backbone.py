...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_layers: Dict,
        ...
        mlp_head_dim = 128
        self.head = nn.Linear(backbone.inplanes, mlp_head_dim) if mlp_head_dim > 0 else nn.Identity()
...
class Joiner(nn.Sequential):
    ...
    def forward(self, input: Tensor):
        ...
        if isinstance(xs, dict):
            out_mlp: List[Tensor] = []
            for name, x in xs.items():
                out_mlp.append(self[0].head(self[0].avgpool(x).squeeze(-1).squeeze(-1)))
                out.append(x)
                # position encoding
                pos.append(self[1](x).to(x.dtype))
            return out_mlp[0], out, pos
        else:
            # for swin Transformer
            out.append(xs[-1])
            pos.append(self[1](xs[-1]).to(xs[-1].dtype))
            return xs[0], out, pos
...
def build_backbone(args):
    ...
    train_backbone = True
    if args.backbone in ['swin_T_224_1k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        imgsize = int(args.backbone.split('_')[-2])
        backbone = build_swin_transformer(args, args.backbone, imgsize, num_classes=args.moco_dim)
        if args.pretrained:
            ...
            if args.fix_backbone:
                for name, parameter in backbone.named_parameters():
                    if 'layers.0' in name or 'layers.1' in name or 'patch' in name:
                        parameter.requires_grad_(False)
