...
parser.add_argument('--core_type', type=str, default='scale', help='path to pretrain_weight?')
    ...
    for batch, (img, (y1, y2, y3, y4), (t_pred_i, t_pred_v, t_pred_t), (feat_i, feat_v, feat_t)) in enumerate(
        ...
        if FLAGS.loss_type == 'i':
            ...
            info_loss = {
                ...
                'hard_loss_i': loss_i.item(),
                'hard_loss_v': loss_v.item(),
                'hard_loss_t': loss_t.item(),
                'hard_loss_ivt': loss_ivt.item(),
...
if FLAGS.pretrain_dir:
    model = load_model(model, pretrain_dir)
