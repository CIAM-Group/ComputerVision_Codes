...
class CholecT50():
    def __init__(self,
        ...
        self.build_train_dataset(trainform, test_fold)
        self.build_val_dataset(testform, test_fold)
        self.build_test_dataset(testform, test_fold)
    ...
    def transform(self):
        ...
        op_test = [transforms.Resize((256, 448)),
                   transforms.Resize((256, 448)), transforms.ToTensor(), normalize, ]
        op_train = [transforms.Resize((256, 448))] + self.augmentation_list + [
    ...
    def build_train_dataset(self, transform, test_fold):
        iterable_dataset = []
        for video in self.train_records:
            dataset = T50(args=self.FLAG, split='train', img_dir=os.path.join(self.dataset_dir, 'data', video),
                          ...
                          transform=transform, test_fold=test_fold)
            iterable_dataset.append(dataset)
        self.train_dataset = ConcatDataset(iterable_dataset)

    def build_val_dataset(self, transform, test_fold):
        iterable_dataset = []
    ...
    def build_test_dataset(self, transform, test_fold):
        iterable_dataset = []
...
class T50(Dataset):
    def __init__(self, args, split, img_dir, triplet_file, tool_file, verb_file, target_file, transform=None,
                 target_transform=None, test_fold=1):
        self.args = args
        self.split = split
        ...
        if self.split == 'train' and self.args.loss_type == 'all':
            ...
            with open('../0-5fold/data_feats/run_{}/k'.format(self.args.teacher_pred_version) + str(
                    test_fold) + '_v_pred.pkl', 'rb') as f:
                self.preds_v = pickle.load(f)[self.img_dir[-2:]]
            with open('../0-5fold/data_feats/run_{}/k'.format(self.args.teacher_pred_version) + str(
                    test_fold) + '_t_pred.pkl', 'rb') as f:
                self.preds_t = pickle.load(f)[self.img_dir[-2:]]

            with open('../0-5fold/data_feats/run_{}/k'.format(self.args.teacher_feat_version) + str(
                    test_fold) + '_i_feats.pkl',
                      'rb') as f:
                self.feats_i = pickle.load(f)[self.img_dir[-2:]]
            with open('../0-5fold/data_feats/run_{}/k'.format(self.args.teacher_feat_version) + str(
                    test_fold) + '_v_feats.pkl',
                      'rb') as f:
                self.feats_v = pickle.load(f)[self.img_dir[-2:]]
            with open('../0-5fold/data_feats/run_{}/k'.format(self.args.teacher_feat_version) + str(
                    test_fold) + '_t_feats.pkl',
                      'rb') as f:
                self.feats_t = pickle.load(f)[self.img_dir[-2:]]
        else:
            self.preds_i = np.zeros((len(self.triplet_labels), 6)).astype(np.float32)
            self.preds_v = np.zeros((len(self.triplet_labels), 10)).astype(np.float32)
            self.preds_t = np.zeros((len(self.triplet_labels), 15)).astype(np.float32)

            self.feats_i = np.zeros((len(self.triplet_labels), self.args.teacher_dim)).astype(np.float32)
            self.feats_v = np.zeros((len(self.triplet_labels), self.args.teacher_dim)).astype(np.float32)
            self.feats_t = np.zeros((len(self.triplet_labels), self.args.teacher_dim)).astype(np.float32)
    ...
    def __getitem__(self, index):
        ...
        return image, (tool_label, verb_label, target_label, triplet_label), (
            self.preds_i[index], self.preds_v[index], self.preds_t[index]), (
                   self.feats_i[index], self.feats_v[index], self.feats_t[index])
