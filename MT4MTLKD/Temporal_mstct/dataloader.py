...
class CholecT50():
    def __init__(self,
        ...
        self.build_train_dataset(testform, model=model, args=args)
        self.build_val_dataset(testform, model=model, args=args)
        self.build_test_dataset(testform, model=model, args=args)
    ...
    def build_train_dataset(self, transform, model, args):
        iterable_dataset = []
        for video in self.train_records:
            dataset = T50(args=args, split='train', img_dir=os.path.join(self.dataset_dir, 'data', video),
    ...
    def build_val_dataset(self, transform, model, args):
        iterable_dataset = []
        for video in self.val_records:
            dataset = T50(args=args, split='val', img_dir=os.path.join(self.dataset_dir, 'data', video),
                          triplet_file=os.path.join(self.dataset_dir, 'triplet', '{}.txt'.format(video)),
                          tool_file=os.path.join(self.dataset_dir, 'instrument', '{}.txt'.format(video)),
                          verb_file=os.path.join(self.dataset_dir, 'verb', '{}.txt'.format(video)),
                          target_file=os.path.join(self.dataset_dir, 'target', '{}.txt'.format(video)),
    ...
    def build_test_dataset(self, transform, model, args):
        iterable_dataset = []
        for video in self.test_records:
            dataset = T50(args=args, split='test', img_dir=os.path.join(self.dataset_dir, 'data', video),
                          triplet_file=os.path.join(self.dataset_dir, 'triplet', '{}.txt'.format(video)),
                          tool_file=os.path.join(self.dataset_dir, 'instrument', '{}.txt'.format(video)),
                          verb_file=os.path.join(self.dataset_dir, 'verb', '{}.txt'.format(video)),
                          target_file=os.path.join(self.dataset_dir, 'target', '{}.txt'.format(video)),
...
class T50(Dataset):
    def __init__(self, args, split, img_dir, triplet_file, tool_file, verb_file, target_file, transform=None,
        ...
        with open(os.path.join('../0-5fold/data_feats/run_' + args.version1,
                               'k' + str(args.kfold) + '_' + args.loss_type + '_feats.pkl'), 'rb') as f:
    ...
    def __getitem__(self, index):
        triplet_label = self.triplet_labels[:, 1:]
        tool_label = self.tool_labels[:, 1:]
        verb_label = self.verb_labels[:, 1:]
        target_label = self.target_labels[:, 1:]
        image = self.feats
        if self.split == 'train':
            num_clips = 256

            random_index = random.choice(range(0, len(image) - num_clips))
            image = torch.tensor(self.feats[random_index: random_index + num_clips: 1]).permute(1, 0).contiguous()

            triplet_label = triplet_label[random_index: random_index + num_clips: 1]
            tool_label = tool_label[random_index: random_index + num_clips: 1]
            verb_label = verb_label[random_index: random_index + num_clips: 1]
            target_label = target_label[random_index: random_index + num_clips: 1]

        else:
            triplet_label = triplet_label[index]

            tool_label = tool_label[index]
            verb_label = verb_label[index]
